...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    conditionalPanel(
      ...
      selectInput("gray_choice", label = "Grayscale Band", choices = c(1, 2, 3, 4), selected = 1)
    ),
...
server <- function(input, output, session) {
  extent_to_use <- eventReactive(input$new_ext, generate_random_window(raster_use_rgb))
